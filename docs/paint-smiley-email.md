...
4. **Select all and copy.** Ctrl+A then Ctrl+C via `send_command` (`shiftdown:ctrl` → `chars:a` →
   `shiftup:ctrl`, then the same for `c`). This puts the canvas bitmap on the clipboard. (`clipboard { action:
   "get" }` can confirm an image is present.)
...
6. **Paste into the body.** `click` the message body, then Ctrl+V. A rich mail composer pastes the bitmap
   inline.
