...
/// error. The HTTP listener lifecycle — including the #152 loopback-bind canonicalization and the
/// #143 Host/Origin/bearer gate — lives in <see cref="McpHttpTransport"/>. Every tool call is loudly
/// audit-logged.
/// </summary>
public static class AgentServer {
    ...
    /// <summary>
    /// The production HTTP transport instance (#215): the listener lifecycle, request gate, and
    /// bounded worker pool live in <see cref="McpHttpTransport"/>; this facade only wires it to the
    /// ambient runtime (settings + dispatcher). Tests construct their own transport with an injected
    /// dispatch delegate instead of the old <c>HttpDispatchOverride</c> static seam.
    /// </summary>
