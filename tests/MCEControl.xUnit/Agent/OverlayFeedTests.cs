...
public class OverlayFeedTests {
    private static readonly DateTime _t0 = new(2026, 6, 30, 12, 0, 0, DateTimeKind.Utc);
    ...
    public void Visible_DropsLinesOlderThanLifetime() {
        ...
        feed.Add(Ev("old"), _t0);
        feed.Add(Ev("fresh"), _t0.AddSeconds(4));

        var visible = feed.Visible(_t0.AddSeconds(6)); // old is 6s (expired), fresh is 2s
    ...
    public void Visible_EmptyWhenAllExpired() {
        ...
        Assert.Empty(feed.Visible(_t0.AddSeconds(3)));
    }
    ...
    public void Visible_ReturnsOldestFirst() {
        ...
        feed.Add(Ev("1"), _t0);
        feed.Add(Ev("2"), _t0.AddSeconds(1));

        var visible = feed.Visible(_t0.AddSeconds(2));
